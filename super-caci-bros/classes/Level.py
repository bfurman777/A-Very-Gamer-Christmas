...
class Level:
    ...
    def loadEntities(self, data):
        try:
            ...
            [self.addDoor(x, y) for x, y in data["level"]["entities"]["Door"]]
        except:
    ...
    def addDoor(self, x, y):
        self.level[y][x] = Tile(self.sprites.spriteCollection.get("Door"), None)
        self.level[y+1][x] = Tile(self.sprites.spriteCollection.get("Door_2"), None)
        self.entityList.append(
            Door(
                self.screen,
                self.sprites.spriteCollection,
